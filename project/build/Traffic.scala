...
class TrafficProject(info: ProjectInfo) extends ParentProject(info) {
  ...
  lazy val traffic_logic = project("traffic-logic", "traffic-logic", info => new TrafficLogic(info))
  lazy val traffic_ui = project("traffic-ui", "traffic-ui", info => new TrafficUI(info))

  class TrafficLogic(info: ProjectInfo) extends DefaultProject(info) {
